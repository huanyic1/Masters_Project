...
def att_process(rank, world_size, lin_reuse_percentage, att_reuse_percentage, model_name, train_dataset, eval_dataset, seed=42):
    # setup(rank, world_size)
    device = torch.device(f"cuda:{rank}")
    ...
    model = respropify_bert_att(base_model, att_reuse_percentage=att_reuse_percentage, lin_reuse_percentage=lin_reuse_percentage, replace_attention=True, replace_linears=True)
    patch_bert_self_attention(model)
    model.to(device)
    # model = DDP(model, device_ids=[rank]) #wrap with DDP

    training_args = TrainingArguments(
        output_dir=f"trainer_out/{model_name.replace('/', '-')}/rp_att_{int(att_reuse_percentage * 100)}_lin_{int(lin_reuse_percentage * 100)}_seed_{seed}/rank_{rank}",
        eval_strategy="steps",
        num_train_epochs=5,
        eval_steps=1/16,
        logging_steps=1/16,
        logging_dir=f"trainer_out/{model_name.replace('/', '-')}/rp_att_{int(att_reuse_percentage * 100)}_lin_{int(lin_reuse_percentage * 100)}_seed_{seed}/rank_{rank}",
        per_device_train_batch_size=8, #adjust batch size to fit on each GPU
        per_device_eval_batch_size=8,
        seed=seed,
        # local_rank=rank, #required for DDP
    ...
    trainer.train()
    if rank == 0:
        torch.save(trainer.state.log_history, f"log_history_att_{att_reuse_percentage}_lin_{lin_reuse_percentage}_seed_{seed}.pt")
    # cleanup()


def att_process_k(rank, world_size, lin_reuse_percentage, att_reuse_percentage, model_name, train_dataset, eval_dataset, k):
    # setup(rank, world_size)
    device = torch.device(f"cuda:{rank}")
    print(f"Process {rank} using device: {device}, Attention Reuse Percentage: {att_reuse_percentage}, Linear Reuse Percentage: {lin_reuse_percentage}")
    ...
    model = resprofify_bert(base_model, reuse_percentage=lin_reuse_percentage)
    model = resprofify_bert_attention(model, reuse_percentage=att_reuse_percentage)
    model.to(device)
    # model = DDP(model, device_ids=[rank]) #wrap with DDP

    training_args = TrainingArguments(
        output_dir=f"trainer_out/{model_name.replace('/', '-')}/rp_att_{int(att_reuse_percentage * 100)}_lin_{int(lin_reuse_percentage * 100)}_k_{k}/rank_{rank}",
        eval_strategy="steps",
        num_train_epochs=5,
        eval_steps=1/16,
        logging_steps=1/16,
        logging_dir=f"trainer_out/{model_name.replace('/', '-')}/rp_att_{int(att_reuse_percentage * 100)}_lin_{int(lin_reuse_percentage * 100)}_k_{k}/rank_{rank}",
        per_device_train_batch_size=8, #adjust batch size to fit on each GPU
        per_device_eval_batch_size=8,
        # local_rank=rank, #required for DDP
    )
    ...
    trainer.train()
    if rank == 0:
        torch.save(trainer.state.log_history, f"log_history_att_{att_reuse_percentage}_lin_{lin_reuse_percentage}_k_{k}.pt")
    # cleanup()
...
if __name__ == "__main__":
    ...
    reuse_percentages = [0, 0.7, 0.9, 0.99]
    # k_s = [2]
    ...
    for reuse_percentage in reuse_percentages:
        # for k in k_s:
        lin_reuse_percentage = reuse_percentage
        att_reuse_percentage = reuse_percentage

        mp.spawn(
            att_process,
            args=(world_size, lin_reuse_percentage, att_reuse_percentage, model_name, train_dataset, eval_dataset, seed),
            nprocs=world_size
        )
        log_histories[f"lin_{lin_reuse_percentage}_att_{att_reuse_percentage}_seed_{seed}"] = torch.load(f"log_history_att_{att_reuse_percentage}_lin_{lin_reuse_percentage}_seed_{seed}.pt")

        mp.spawn(
            process,
            args=(world_size, reuse_percentage, model_name, train_dataset, eval_dataset, seed),
            nprocs=world_size
        )
        log_histories[f"{reuse_percentage}_linear"] = torch.load(f"log_history_{reuse_percentage}.pt")



    plot_log_histories(log_histories, file_name=f"resprop_both.png")
